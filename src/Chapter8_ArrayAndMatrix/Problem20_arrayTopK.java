...
public class Problem20_arrayTopK {
	...
	public static class HeapNode{  //HeapNode类
	...
	public void heapInsert(HeapNode [] heap, int index){//建堆时的调整过程
		while(index !=0){
	...
	public void heapify(HeapNode [] heap, int index, int heapSize){//调整为大根堆
		int left = index*2+1;
		int right = index*2+2;
		int largest = index;
		while(left<heapSize){
